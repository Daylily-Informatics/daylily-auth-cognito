...
def add_google_idp(
    ...
    google_client_json: str | None = typer.Option(None, "--google-client-json", help="Path to Google OAuth client JSON"),
    ...
    if not app_name and not client_id:
        ...
        raise typer.Exit(1)
    admin, _runtime = _get_admin_client(profile=profile, region=region, require_config=True, require_required_keys=True, require_profile=True)
...
def register(registry: CommandRegistry, spec: CliSpec | None = None) -> None:
    del spec
    registry.add_command(None, "add-google-idp", add_google_idp, help_text="Configure Google IdP on a pool and enable it on an app client.", policy=MUTATE_POLICY)
    registry.add_command(None, "setup-with-google", setup_with_google, help_text="Provision Cognito and configure Google IdP.", policy=MUTATE_POLICY)
    registry.add_command(None, "setup-google", setup_google, help_text="Write Google OAuth credentials into the active config file.", policy=MUTATE_POLICY)
